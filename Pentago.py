...
def _get_all_5_window_cells(board: np.ndarray):
    ...
    for c in range(6):
        for s in range(2): yield [(s+i, c) for i in range(5)]
    for r in range(2):
        for c in range(2): yield [(r+i, c+i) for i in range(5)]
    for r in range(2):
...
def _score_window_for(window: np.ndarray, player: int) -> int:
    ...
    if ai_n > 0 and opp_n > 0: return 0
...
class PentagoAI:
    ...
    def evaluate_board(self, board: np.ndarray) -> int:
        ...
        four_threats = 0
        for window in _get_all_5_windows(board):
            ai_score  = _score_window_for(window, self.ai_player)
            opp_score = _score_window_for(window, self.human_player)
            score += ai_score
            score -= opp_score * 2
            ai_n  = int(np.sum(window == self.ai_player))
            opp_n = int(np.sum(window == self.human_player))
            if ai_n == 4 and opp_n == 0:
                four_threats += 1
    ...
    def _is_critical_threat(self, board: np.ndarray, player: int) -> tuple:
        ...
        for window_cells in _get_all_5_window_cells(board):
            ...
            p_n   = vals.count(player)
            opp_n = vals.count(opp)
            emp_n = vals.count(EMPTY)
            if p_n == 4 and opp_n == 0 and emp_n == 1:
    ...
    def _best_block_move(self, board: np.ndarray, threat_rc: tuple) -> tuple:
        ...
        for q in range(4):
            for d in (1, -1):
                ...
                for window in _get_all_5_windows(nb):
                    ai_n  = int(np.sum(window == self.ai_player))
                    opp_n = int(np.sum(window == self.human_player))
                    if ai_n == 4 and opp_n == 0:
                        s += _FOUR_SCORE // 2
    ...
    def alpha_beta(self, board: np.ndarray, depth: int, alpha: float, beta: float, maximizing: bool, deadline: float = math.inf, current_player: int = None):
        ...
        next_player = (self.human_player if current_player == self.ai_player else self.ai_player)

        moves = self._order_moves(legal_moves(board), board, maximizing, current_player)
        ...
        if maximizing:
            ...
            for move in moves:
                ...
                nb = apply_move(board, move, current_player)
                score, _ = self.alpha_beta(nb, depth - 1, alpha, beta, False, deadline, next_player)
                if score > max_eval:
            ...
            for move in moves:
                if _time.monotonic() > deadline: break
                nb = apply_move(board, move, current_player)
                score, _ = self.alpha_beta(nb, depth - 1, alpha, beta, True, deadline, next_player)
                if score < min_eval:
    ...
    def get_best_move(self, board: np.ndarray, depth: int = 4, time_limit: float = 3.5) -> tuple:
        ...
        own_critical = self._is_critical_threat(board, self.ai_player)
        if own_critical is not None:
            r, c = own_critical
            best_score, best_move = -math.inf, None
            for q in range(4):
                for d in (1, -1):
        ...
        critical_cell = self._is_critical_threat(board, self.human_player)
        if critical_cell is not None:
            return self._best_block_move(board, critical_cell)

        deadline  = _time.monotonic() + time_limit
        best_move = None
        for d in range(1, depth + 1):
            if _time.monotonic() >= deadline: break
            _, move = self.alpha_beta(board, d, -math.inf, math.inf, True, deadline, self.ai_player)
            if move is not None:
                best_move = move
...
_UI_RESERVE  = max(200, min(260, _sh // 3))

CELL_SIZE    = min(82, (_sh - _UI_RESERVE) // 6)
CELL_SIZE    = max(40, CELL_SIZE)

BOARD_SIZE   = CELL_SIZE * 6

SIDE_PANEL   = max(120, min(180, _sw // 8))

WIDTH        = min(_sw, BOARD_SIZE + SIDE_PANEL * 2)
HEIGHT       = min(_sh, BOARD_SIZE + _UI_RESERVE)
MARGIN_X     = (WIDTH - BOARD_SIZE) // 2
MARGIN_Y     = max(80, HEIGHT // 7)
...
font_title   = load_font("Cinzel.ttf",   34)
font_heading = load_font("Cinzel.ttf",   20)
font_label   = load_font("Cinzel.ttf",   13)
font_ui      = load_font("Raleway.ttf",  16)
font_ui_sm   = load_font("Raleway.ttf",  12)
font_symbol  = pygame.font.SysFont("segoeuisymbol", 18)
...
def draw_marble(surface, cx, cy, radius, player):
    ...
    pygame.draw.circle(shadow_surf, (0, 0, 0, 80), (int(radius+4), int(radius+6)), int(radius))
    ...
    for ri in range(int(shine_r), 0, -1):
        ...
        pygame.draw.circle(shine_surf, (*shine_col, alpha), (int(shine_r*2), int(shine_r*2)), ri)
...
def draw_board(surface, game: GameState, animating_quad=-1):
    ...
    pygame.draw.rect(surface, C_QUAD_BORDER, pygame.Rect(*board_rect), 3, border_radius=12)
    ...
    for q in range(4):
        ...
        pygame.draw.rect(surface, C_QUAD_BORDER, pygame.Rect(*quad_rect), 1, border_radius=8)
    ...
    pygame.draw.line(surface, C_DIVIDER, (MARGIN_X + 4, ly), (MARGIN_X + BOARD_SIZE - 4, ly), 4)
    pygame.draw.line(surface, C_DIVIDER, (lx, MARGIN_Y + 4), (lx, MARGIN_Y + BOARD_SIZE - 4), 4)
...
class Button:
    ...
    def draw(self, surface):
        ...
        draw_gradient_rect(surface, self.rect, face, tuple(max(0, c - 20) for c in face))
        pygame.draw.rect(surface, border, self.rect, 2, border_radius=8)
        inner = self.rect.inflate(-4, -4)
        pygame.draw.line(surface, tuple(min(255, c + 30) for c in face), (inner.x, inner.y), (inner.right, inner.y), 1)
...
def main():
    ...
    grid_x0 = MARGIN_X + (BOARD_SIZE - grid_total_w) // 2
    ...
    for qi in range(4):
        ...
        label    = f"Q{qi+1}"
        rot_buttons.append(Button(bx_cw,  by, btn_w, btn_h, f"{label} ↻", (qi,  1), symbol=True))
        rot_buttons.append(Button(bx_ccw, by, btn_w, btn_h, f"{label} ↺", (qi, -1), symbol=True))

    side_x = MARGIN_X + BOARD_SIZE + 24
    surrender_btn = Button(side_x, MARGIN_Y + 20, 140, 40, "Surrender", danger=True)
    main_menu_btn = Button(side_x, MARGIN_Y + 70, 140, 40, "Main Menu", danger=False)

    next_match_btn   = Button(side_x, MARGIN_Y + 20, 140, 46, "Next Match")
    restart_menu_btn = Button(side_x, MARGIN_Y + 76, 140, 46, "Main Menu")
    full_restart_btn = Button(side_x, MARGIN_Y + 20, 140, 46, "Main Menu")
    ...
    while running:
        ...
        if app_state == "MENU":
            ...
            draw_decorative_line(screen, frame.x + 20, frame.y + 8, frame.right - 20, frame.y + 8)
            draw_decorative_line(screen, frame.x + 20, frame.bottom - 8, frame.right - 20, frame.bottom - 8)

            draw_gold_text(screen, "PENTAGO", font_title,  WIDTH//2, HEIGHT//2 - 240)
            draw_gold_text(screen, "Game Setup", font_heading, WIDTH//2, HEIGHT//2 - 205, color=C_TEXT_MID)
            ...
            draw_gradient_rect(screen, input_box, C_BTN_FACE, tuple(max(0, c - 10) for c in C_BTN_FACE))
            pygame.draw.rect(screen, ib_color, input_box, 2, border_radius=6)
            name_surf = font_ui.render(player_name + ("|" if active_input else ""), True, C_TEXT_BRIGHT)
            ...
            active_col_rect = (color_btn_white.rect if base_human_color == WHITE else color_btn_black.rect)
            ...
            draw_gold_text(screen, f"You are {c_str}  ·  Best of {best_of_series}", font_ui_sm, WIDTH//2, 80, color=C_TEXT_DIM)
            ...
            if app_state == "PLAYING" and game.turn == current_ai_color and not animating:
                if game.phase == "place":
                    ...
                    pad = 16
                    bg = pygame.Surface((think_surf.get_width() + pad*2, think_surf.get_height() + pad), pygame.SRCALPHA)
                    bg.fill((0, 0, 0, 160))
                    screen.blit(bg, (think_rect.x - pad, think_rect.y - pad // 2))
                    screen.blit(think_surf, think_rect)
                    pygame.display.flip()

                    r, c, q, d = ai.get_best_move(game.board, depth=4, time_limit=5.0)
                    game.place(r, c)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if app_state == "PLAYING" and not animating:
                        if surrender_btn.rect.collidepoint(event.pos):
                            ...
                            app_state = ("SERIES_OVER" if ai_wins == target_wins else "ROUND_OVER")
